...
/**
 * The conversation, for any application that has one.
 *
 * Two statuses get more than a badge. `WAITING_APPROVAL` is not an error and
 * not a hang: the run proposed a write, stopped at the policy gate, and is
 * waiting for a person — so the pane says so and points at the inbox.
 * `NEEDS_ATTENTION` is the honest end of a call that was dispatched but whose
 * outcome was never confirmed, and the worst thing the UI could do there is
 * invite a retry as if nothing had happened.
 */
export default function AppConversationPane({
  ...
  return (
    <section
      className="research-pane research-pane-conversation"
      aria-label={t("appThread.conversation.paneTitle")}
    >
      <div className="research-pane-head">
        <h2>{t("appThread.conversation.paneTitle")}</h2>
        <button
          type="button"
          className="button button-ghost"
          onClick={turns.reload}
          disabled={turns.loading}
        >
          {t("common.refresh")}
        </button>
      </div>
      ...
      <div className="research-pane-body">
        ...
        {turns.loaded && !turns.error && turnList.length === 0 && pendingInput === null && (
          <EmptyState
            title={t("appThread.conversation.empty")}
            hint={t("appThread.conversation.emptyHint")}
          />
        )}
                ...
                <div className="conversation-message conversation-message-agent">
                  ...
                  {turn.status === "WAITING_APPROVAL" && (
                    <div className="state-block state-inline">
                      <p className="state-title">{t("appThread.conversation.waitingApproval")}</p>
                      <p className="state-hint">{t("appThread.conversation.waitingApprovalHint")}</p>
                      <Link className="button button-primary" href="/approvals">
                        {t("appThread.conversation.openApprovals")}
                      </Link>
                    </div>
                  )}
                  {turn.status === "NEEDS_ATTENTION" && (
                    <div className="state-block state-inline">
                      <p className="state-title">{t("appThread.conversation.needsAttention")}</p>
                      <p className="state-hint">{t("appThread.conversation.needsAttentionHint")}</p>
                    </div>
                  )}
                  ...
                  */}
                  {turn.agent_run_id ? (
                    <p className="conversation-run">
                      ...
                      >
                        {t("appThread.conversation.runLabel", { id: shortId(turn.agent_run_id) })}
                      </Link>
