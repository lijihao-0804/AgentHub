...
| `uv run --locked pytest` | PASS | 23 passed |
| `uv run --locked pytest -m integration -rs` | PASS | 5 个真实 PostgreSQL 集成用例通过，18 个非 integration 用例 deselected |
...
Redis/Qdrant dependency smoke test 仍按计划延后至 M3；LangGraph PostgreSQL checkpoint
bootstrap 与 API restart 后 WAITING_APPROVAL resume 仍按计划延后至 M5。当前 M1 结论为
`PASS`。
