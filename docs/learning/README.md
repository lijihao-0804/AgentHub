...
| | `docs/report/`（19 章） | `docs/learning/`（本目录） |
|---|---|---|
...
| [03-domain-model-lifecycle.md](03-domain-model-lifecycle.md) | **8 个核心对象的生命周期**（不是字段表）。重点回答：为什么有些东西可变，有些东西必须不可变，以及「不可变」到底是谁在拦 | 02 读完，想把表结构串起来时 |
| [04-runtime-labs.md](04-runtime-labs.md) | **10 个实验**，没有理论。改一个变量 → 预测 → 跑 → 记录真实行为 → 解释 | 03 之后。也可以在 02 卡住的地方随时插进来 |
...
阶段 6  回到 docs/report/03 和 08     此时那两章会变得很好读
```

阶段 6 不是凑数。`docs/report/03-核心机制详解.md` 里有 11 个技术亮点，
第一次读的时候它们是 11 条并列的结论；
走完前五个阶段之后再读，它们会变成 11 个你已经亲手碰过的位置。
...
实验会写数据库。这是本机开发库，没关系——
但不要在实验里去动 `.env`，所有可调的东西都能从界面或 API 改。
...
| `09-failure-casebook.md` | 失败码逐条：怎么复现、怎么读、该怎么处理 | 04 章已经用实验覆盖了最重要的四类（成本、上下文、UNKNOWN_OUTCOME、断流）。剩下的等实验里真撞到再补，凭想象编案例没有价值 |
| `10-resume-extraction.md` | 从项目里提炼简历条目 | **现在不该写。** 简历句子是学习的副产品，不是学习的目标；先写它会让人去背结论而不是读代码 |

如果某一天要补，优先级是 `05 → 06 → 09`，
而且触发条件应该是「我在做那件事的时候发现缺手册」，不是「目录看起来不齐」。
