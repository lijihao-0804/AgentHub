...
执行图只有 8 个节点（查看 `packages/agent_runtime/runtime.py` 中 `_AgentRunGraph` 的图构造与边定义；
LangGraph 的图定义在 `packages/agent_runtime/adapters/langgraph/runtime.py`）：
...
```python
# packages/agent_runtime/runtime.py — `_AgentRunGraph.prepare`
...
——当前新写入是**带内容哈希的对象，不是裸数组**；历史 ID-only 快照仍兼容读取。
