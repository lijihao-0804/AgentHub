...
12 个机制。每一个都能单独聊 10 分钟，每一个都有代码位置可以当场翻。
...
**位置**：`packages/agent_runtime/context_budget.py`（1058 行）
...
可投影（按类别上限裁剪）：      RAG_EVIDENCE / TOOL_RESULT
可驱逐：                      CONVERSATION
```
...
**位置**：`packages/agent_runtime/context_budget.py:921`
...
对非 `TOOL_RESULT` 的类别，代码是**读取** payload 里已有的 `trust` / `data_trust`；
对 `TOOL_RESULT`，是**无条件覆盖**。
...
| 持久中断 | interrupt + checkpoint | 审批挂住进程 / 丢状态 | `runtime.py:1596` |
...
| 信任盖章 | 工具结果强制 UNTRUSTED | payload 自我提权 | `context_budget.py:921` |
| 结构化截断 | JSON 投影 + truncated 标记 | 模型以为看到了全部 | `context_budget.py:906` |
