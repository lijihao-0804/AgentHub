...
**位置**：`packages/agent_runtime/publish.py`、`frozen.py`、`packages/agent_runtime/runtime.py` 的 `_AgentRunGraph.prepare`
...
**位置**：`packages/agent_runtime/runtime.py` 的 `policy()`、`packages/approvals/service.py`
...
意义：快照固定的是可参与检索的**文档修订集合**，不是固定某条查询最终命中的 chunk、排序或模型回答。
同一快照让知识语料的变化可被隔离；嵌入/索引状态、检索与重排实现、查询本身以及模型仍可能影响结果。
如果只绑定 `LATEST`，跨时间的实验定义还可能悄悄指向不同文档修订，无法把知识变更与其他因素分开。
...
1. **分层的实验身份哈希**（`reproducibility.py`）
   `evaluator_manifest` 记录 schema 与 evaluator 版本，若启用 judge 还冻结其非密钥配置；
   manifest 本身不含 PricingSnapshot。变体哈希另绑定 AgentVersion/resolved spec、知识快照、
   PricingSnapshot 与 variant metadata。finalize 后的 experiment spec 再组合数据集版本/内容哈希、
   split、purpose、重复次数、build SHA、变体与 evaluator manifest。哈希相同表示这些**持久化定义相同**，
   不是保证模型输出、检索顺序或最终指标数值必然相同。
...
**位置**：`packages/agent_runtime/context_budget.py:962` 的 JSON 投影与信任字段处理路径
...
| 存储 | `workspace_memories`，`ACTIVE` / `SUPERSEDED` / `INVALIDATED` 三态 | 保留未来可信语义替换与人工停用的不同审计含义；当前自动抽取不创建 `SUPERSEDED` |
| 注入 | **一条** system 消息装全部记忆，强制 `UNTRUSTED`，落 `MEMORY` 类别 | 一条一个会被裁掉一半，留给模型一个它无从察觉的残缺信念集；落 `SYSTEM_PROMPT` 会把用户的问题挤出去 |
| 快照 | 首次 PREPARE 冻结 id 与内容哈希进 `effective_memory_snapshot`，之后按冻结顺序装载并校验哈希；历史 ID-only 快照兼容 | 重放回答的就变成「它现在会拿到什么」，而不是「它当时拿到了什么」——可解释性当场归零 |
...
这里的「可复现」严格指同一个 Run 恢复时能重新装载并校验它第一次实际选中的记忆输入，
不是跨新 Run 保证再次选中相同记忆，也不是保证外部模型输出逐字相同。正式 Experiment 的变体哈希
不包含每个 query 的 memory hit；evaluation worker 会读取记忆并把选择结果写入各 AgentRun 的快照，
但不会从评测 Run 写回记忆。比较记忆实验时必须检查这些逐 Run 快照。

---

## 15. 知识检索模型预热：未预热的冷进程第一次请求会超过工具预算
...
当 `knowledge_warm_models_on_start` 关闭（默认值）且进程/模型缓存是冷的，第一次 `search_knowledge`
要在**工具自己的 30 秒预算之内**
把 BGE-M3 和 cross-encoder 两个模型加载完——实测约 35 秒。
在记录的本机配置下，模型加载约 35 秒，超过工具预算，所以**未预热部署的第一次检索会超时**；
启用并成功完成启动预热后不应套用这个结论。
...
| 持久中断 | interrupt + checkpoint | 审批挂住进程 / 丢状态 | `runtime.py` 的 `policy` / `AgentRunService.resume` |
| 同 Run 恢复 | resume 而非新建 | 审计要跨 Run 拼 | `runtime.py` 的 `AgentRunService.resume` |
...
| 信任盖章 | 工具结果与记忆强制 UNTRUSTED | payload 自我提权 | `context_budget.py:962` |
| 结构化截断 | JSON 投影 + truncated 标记 | 模型以为看到了全部 | `context_budget.py:950` |
...
| 记忆快照 | 首次 PREPARE 冻结 id 与内容哈希，之后校验并按冻结顺序装载；旧 ID-only 快照兼容 | 开了记忆的 Run 不可重放 | `agent_runtime/models.py:330` |
