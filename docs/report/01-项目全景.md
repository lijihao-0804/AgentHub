...
| 迁移 | Alembic（29 个版本，单一 head） | 「绝不制造 multiple heads」是硬约束 |
...
Next.js 15.2.4（App Router）+ React 19.1.0 + TypeScript 5.8.2，111 个 ts/tsx 文件共 25,317 行。
零 UI 框架、零组件库，纯手写 CSS——14 个样式文件共 1,726 行。
...
- pytest，105 个测试文件 / 26,943 行，`pytest tests/unit -q` **890 passed**，单元 / 集成分离（集成需真实 PostgreSQL URL）
...
│  │                          #   （记忆以单条 system 消息注入；上下文裁剪对模型可见）
│  ├─ approvals/              # 审批状态机、对账
│  ├─ threads/                # 会话与轮次
│  ├─ artifacts/              # 产出物：schema / 投影 / 记录器
│  ├─ memory/                 # 长期记忆：抽取、存储、选择、失效
│  ├─ agent_templates/        # 四个应用模板
│  ├─ evaluation/             # 数据集、实验、指标、消融、发布闸门
│  └─ observability/          # 运行观测投影
├─ migrations/versions/       # 0001 → 0029，单链
├─ tests/{unit,integration}/  # 105 个测试文件、单元 890 passed
├─ benchmarks/                # 检索评估基线数据
...
timeline
    ...
    四应用 : Incident / Data Analyst / Support 上同一套运行时
    自证 : 真机验证 → 前端实测 → 语料对抗 → 生命周期修复
    第一档 : Run 脱离 HTTP 连接 → judge 冻结 → 成本闸门 → 外部审查整改
    长期记忆 : 阶段 0 裁剪可见化 / 1 会话内检索 / 2 长期记忆与快照 / 3 记忆管理（两个开关默认关）
```
...
- 最后一段「自证」不是收尾，是四轮**对抗性验证**，
  挖出的问题比前面任何一个里程碑都多（见 [00 章第七幕](00-从零到现在.md)）。
- 「长期记忆」这一幕是唯一**推翻了先前一条明确取舍**的里程碑：
  06 章 §1.4 原本论证过「不做长期记忆」，理由是它会毁掉可复现性。
  所以这一幕的顺序是先把「选中的记忆冻进 `effective_memory_snapshot`」做掉，
  再让 agent 开始记东西——**可重放是准入条件，不是配套设施**（见 [19 章](19-长期记忆与上下文管理实施报告.md)）。

> **这一节只给骨架。** 每一幕「被什么问题逼出来、做完又暴露了什么」的完整因果链，
> 在 **[00 · 从零到现在](00-从零到现在.md)** ——那一章是全书主干，
> 也是回答「你为什么需要这么多东西」的地方。

## 6. 规模数据（可以直接报的数字）

| 指标 | 数值 |
|---|---|
| Python 文件（apps+packages+migrations） | 256（连测试 369） |
| Python 代码行（apps + packages + migrations） | 48,728 |
| 前端 ts/tsx 文件 | 111（25,317 行） |
| 前端手写 CSS 行数 | 1,726（14 个文件，零框架） |
| 测试文件 | 105（`pytest tests/unit` 890 passed，26,943 行，为产品代码的 55%） |
| Alembic 迁移 | 29（单 head，其中 10 个不新建任何表） |
| API 路由模块 | 15 |
