...
- pytest，96 个测试文件 / 703 个用例 / 24,703 行，单元 / 集成分离（集成需真实 PostgreSQL URL）
...
├─ migrations/versions/       # 0001 → 0026，单链
├─ tests/{unit,integration}/  # 96 个测试文件、703 个用例
├─ benchmarks/                # 检索评估基线数据
...
| 指标 | 数值 |
|---|---|
| Python 文件（apps+packages+migrations） | 237（连测试 340） |
| Python 代码行（apps + packages + migrations） | 44,295 |
| 前端 ts/tsx 文件 | 113（24,392 行） |
| 前端手写 CSS 行数 | 1,726（14 个文件，零框架） |
| 测试文件 | 96（703 个用例，24,703 行，为产品代码的 56%） |
| Alembic 迁移 | 26（单 head，其中 9 个不新建任何表） |
| API 路由模块 | 15 |
