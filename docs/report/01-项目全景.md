...
| 迁移 | Alembic（26 个版本，单一 head） | 「绝不制造 multiple heads」是硬约束 |
...
Next.js 15.2.4（App Router）+ React 19.1.0 + TypeScript 5.8.2，113 个 ts/tsx 文件共 24,392 行。
...
- pytest，99 个测试文件 / 744 个用例（pytest 收集 1,000，含参数化展开）/ 25,564 行，单元 / 集成分离（集成需真实 PostgreSQL URL）
...
├─ migrations/versions/       # 0001 → 0027，单链
├─ tests/{unit,integration}/  # 99 个测试文件、744 个用例
├─ benchmarks/                # 检索评估基线数据
...
| 指标 | 数值 |
|---|---|
| Python 文件（apps+packages+migrations） | 243（连测试 350） |
| Python 代码行（apps + packages + migrations） | 46,383 |
| 前端 ts/tsx 文件 | 113（24,595 行） |
| 前端手写 CSS 行数 | 1,726（14 个文件，零框架） |
| 测试文件 | 99（744 个用例，25,564 行，为产品代码的 55%） |
| Alembic 迁移 | 27（单 head，其中 9 个不新建任何表） |
| API 路由模块 | 15 |
