...
| **记忆是独立工程学科** | 2026 已有基准（LoCoMo）、21 个框架；多数团队自建「薄记忆层 = KV + 摘要压缩」 | ❌ 无。`packages/threads/context.py:4` 明写「no summarization, no embedding and no memory」 |
...
#### A. 让 Run 脱离 HTTP 连接（**本文唯一一条架构级建议**）
...
#### D. 把「不做 Memory / 不做多 Agent」从立场升级成设计（写 ADR，不实现）
...
- 产出：`docs/adr/0011-memory-must-be-snapshotted.md`，1~2 小时，0 行代码，0 风险
...
- **OTel** 是管道工程。2026 的语义约定已定下 `invoke_agent` → `chat` / `execute_tool`
  ...
  但它**不改变系统能做什么**。
- **Playwright E2E** 是卫生，不是能力。「后端 96 文件 703 用例、前端 0」
